...
\title{\textbf{Teleology as Covariant Boundary Selection in a Consciousness--Ethics Field Theory}}
\author{Christopher M. Baird\thanks{ZoraASI Research Group.}\\ Doctorate of Metaphysics, University of Sedona}
\date{February 2026}
...
This paper develops an alternative that is both conservative (bulk dynamics stays standard and unitary) and structurally explicit: teleology is represented by a covariant \emph{boundary selection functional} at a future hypersurface, or equivalently by a covariant \emph{final weighting operator} used only in the rule that maps amplitudes to outcome probabilities. The resulting framework can reproduce an ``ethically biased Born rule'' of the form
\begin{equation}
P(i) \propto |c_i|^2\,\exp\big(-\mathcal{E}_i/C\big),
\label{eq:biased-born-intro}
\end{equation}
while maintaining unitary Tomonaga--Schwinger evolution for states on arbitrary spacelike hypersurfaces.
...
\begin{equation}
\bra{i}\rho_f\ket{i}\propto \exp\Big(-\mathcal{E}_i/C\Big),
\label{eq:diagweight}
\end{equation}
then~\eqref{eq:PTBorn} reduces to the biased Born rule~\eqref{eq:biased-born-intro}.

A natural identification in the present theory is that $\mathcal{E}_i$ is the branch-evaluated ethical functional induced by $E(x)$ and $\Phi_c(x)$, e.g.,
\begin{equation}
\mathcal{E}_i \equiv -\lambda\,\Gamma_i
...
In the limit $\lambda\to 0$, standard QFT probabilities are recovered. For $\lambda\ne 0$, deviations appear only in contexts where (i) branch structure is well-defined (decoherence) and (ii) $\rho_f$ has nontrivial overlap structure with outcome projectors. The magnitude and operational detectability of deviations are model-dependent and can be constrained by no-signaling requirements and existing tests of quantum statistics.
